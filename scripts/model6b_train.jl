...
model_name = "model6b_l"*string(nlayers)*"_lr"*string(lr)*"_h"*string(nhidden)*"_bs"*string(bs)*"_tversky"*string(tversky_beta)
save_path = joinpath(workdir, "models", dataset_name, model_name)
# delete all checkpoints in save directory
if isdir(save_path)
    println("Checkpoint directory '$(save_path)' exists. Deleting")
...
if isdir(test_solve_output_dir)
    println("Solve output directory '$(test_solve_output_dir)' exists. Deleting")
    rm(test_solve_output_dir, recursive=true, force=true)
end
# default solver
output_dir_default = joinpath(test_solve_output_dir, "default")
if isdir(output_dir_default)
    println("Default solve output directory '$(output_dir_default)' exists. Deleting")
...
# model
model = UMFSolver.M6ClassifierModel(nhidden, 3, nlayers, 71) |> device
...
function loss(g::GNNGraph)
    ...
    #g = g |> device
    #println("in loss: ", [typeof(g.e), typeof(g.targets)])
