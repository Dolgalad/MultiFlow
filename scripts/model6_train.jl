...
model_name = "model6_l"*string(nlayers)*"_lr"*string(lr)*"_h"*string(nhidden)*"_bs"*string(bs)*"_tversky"*string(tversky_beta)
...
function solve_test_dataset(epoch, history)
	...
	s1,ss1 = solveUMF(inst,"CG","highs","./model6_test_output.txt", "", "clssp $checkpoint_path 1")
    end
end
