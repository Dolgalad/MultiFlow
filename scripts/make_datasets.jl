...
for (inst, full_dir_path) in zip(instances[idx], instance_paths[idx])
    # dataset name
    dataset_name = "dataset_"*replace(replace(full_dir_path, base_instance_dir => ""), "/" => "_")
    ...
    UMFSolver.make_dataset(inst, n_train, dataset_path)
    # test dataset
    dataset_path = joinpath(dataset_dir, dataset_name*"_test")

    UMFSolver.make_dataset(inst, n_test, dataset_path)
    println()
end
