...
tversky_beta = 0.2
es_patience = 50
dataset_path = joinpath(workdir, "datasets/dataset_prc_small_flexE_1_test")
dataset_name = basename(dataset_path)

# model name and save path
model_name = "model3_l"*string(nlayers)*"_lr"*string(lr)*"_h"*string(nhidden)*"_bs"*string(batchsize)*"_tversky"*string(tversky_beta)
...
# model
model = UMFSolver.M3ClassifierModel(nhidden, 3, nlayers, 71) |> device
