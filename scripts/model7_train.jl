...
tversky_beta = 0.9f0
es_patience = 100
dataset_path = joinpath(workdir, "datasets/dataset_prc_small_mixed_1_train")
dataset_name = basename(dataset_path)
test_dataset_path = joinpath(workdir, "datasets/dataset_prc_small_mixed_1_test")

# model name and save path
model_name = "model7_l"*string(nlayers)*"_lr"*string(lr)*"_h"*string(nhidden)*"_bs"*string(bs)*"_tversky"*string(tversky_beta)
...
# model
model = UMFSolver.M7ClassifierModel(nhidden, 3, nlayers, 71) |> device
...
function loss(g::GNNGraph)
    #println("in loss : ", [p for p in Flux.params(model)])
    #println("\t", [typeof(g.targets), typeof(g.edata.e)])
