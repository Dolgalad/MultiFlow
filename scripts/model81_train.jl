"""Train M81ClassifierModel : same as M8ClassifierModel but with a Dense layer as scoring
Arguments : 
    - train dataset path
    - test dataset path
    - batch size
...
model_name = "model81_l"*string(nlayers)*"_lr"*string(lr)*"_h"*string(nhidden)*"_bs"*string(bs)*"_e"*string(epochs)*"_tversky"*string(tversky_beta)
...
# model
model = UMFSolver.M81ClassifierModel(nhidden, 4, nlayers, nnodes) |> device
