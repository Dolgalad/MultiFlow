...
model_name = "model4_l"*string(nlayers)*"_lr"*string(lr)*"_h"*string(nhidden)*"_bs"*string(batchsize)*"_tversky"*string(tversky_beta)
...
# model
model = UMFSolver.M4ClassifierModel(nhidden, 3, nlayers, 71) |> device
