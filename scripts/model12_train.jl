"""Train M12ClassifierModel : no graph embedding
Arguments : 
    - train dataset path
    - test dataset path
    - batch size
...
if length(ARGS)>=1
    ...
    network = "fr_random_regular_digraph_10"
    dataset_name = network
    dataset_path = joinpath(workdir, "datasets", dataset_name, "train")
    test_dataset_path = joinpath(workdir, "datasets", dataset_name, "test")
end

# model name and save path
model_name = "model12_l"*string(nlayers)*"_lr"*string(lr)*"_h"*string(nhidden)*"_bs"*string(bs)*"_e"*string(epochs)*"_tversky"*string(tversky_beta)
...
inst = UMFSolver.scale(UMFData(joinpath(test_dataset_path, "1"), edge_dir=:single))
config = set_config("CG","cplex","./output","linear","dijkstra")
...
for inst_dir in readdir(test_dataset_path, join=true)
    if UMFSolver.is_instance_path(inst_dir)
        local inst = UMFSolver.scale(UMFData(inst_dir, edge_dir=:single))
        #s1,ss1 = solveUMF(inst,"CG","cplex",joinpath(output_dir_default, basename(inst_dir))*".json")
        s1,ss1 = solveCG(inst,config)
    end
end

print("Loading dataset...")
all_graphs = UMFSolver.load_dataset(dataset_path, edge_dir=:single)
all_graphs = map(UMFSolver.aggregate_demand_paths, all_graphs)
...
# model
model = UMFSolver.M12ClassifierModel(nhidden, 4, nlayers, nnodes) |> device
