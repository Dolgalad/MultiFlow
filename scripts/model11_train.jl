"""Train M11ClassifierModel : same as M11ClassifierModel but with a Dense layer as scoring
Arguments : 
    - train dataset path
    - test dataset path
    - batch size
...
model_name = "model11_l"*string(nlayers)*"_lr"*string(lr)*"_h"*string(nhidden)*"_bs"*string(bs)*"_e"*string(epochs)*"_tversky"*string(tversky_beta)
...
# model
model = UMFSolver.M11ClassifierModel(nhidden, 4, nlayers, nnodes) |> device
