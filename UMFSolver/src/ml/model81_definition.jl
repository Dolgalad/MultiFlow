...
Flux.@functor M81MPLayer

function M81MPLayer(n::Int; drop_p::Float64=.1)
    return M81MPLayer(UMFSolver.MyMEGNetConv3(n), 
		      Flux.Dropout(drop_p), 
		      Flux.Dropout(drop_p),
		      Flux.BatchNorm(n), 
		      Flux.BatchNorm(n), 
...
Flux.@functor M81ClassifierModel

#function sum_gxe(a, b)
#    return [a[1], a[2]+b[2], a[3]+b[3]]
#end

function M81ClassifierModel(node_feature_dim::Int, 
    ...
    graph_conv      = [UMFSolver.M81MPLayer(node_feature_dim, drop_p=drop_p) for _ in 1:n_layers]
    ...
    scoring = Flux.Dense(2*node_feature_dim => 1)

    M81ClassifierModel(node_embeddings, edge_encoder, demand_encoder, graph_conv, demand_mlp, edge_mlp, scoring, device)
end


function compute_graph_embeddings(model::M81ClassifierModel, g::GNNGraph)
    #println("1")
    if g.num_graphs==1
...
function make_demand_codes(model::M81ClassifierModel, g::GNNGraph)
...
function (model::M81ClassifierModel)(g::GNNGraph)
